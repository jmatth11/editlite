...
void handle_row_scroll(struct display *d, struct display_dim dim) {
  ...
  if (d->cursor.pos.row >= cur_page->lines.len) {
    d->cursor.pos.row = cur_page->lines.len - 1;
  } else if (d->cursor.pos.row >= cur_page->row_offset + dim.row) {
    cur_page->row_offset++;
  } else if (d->cursor.pos.row < cur_page->row_offset) {
    cur_page->row_offset = d->cursor.pos.row;
  }
}
